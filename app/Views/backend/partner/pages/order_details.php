                                    ...
                                    <div class="mb-6">
                                        <div class=" d-flex items-center justify-content-between mb-2 text-dark">
                                            <div>
                                                ...
                                                <div class="tag border-0 rounded-md ltr:ml-2 rtl:mr-2 bg-cyan-100 text-cyan-600 dark:bg-cyan-500/20 dark:text-cyan-100"><?= (!empty($order_details['payment_status'])) ? labels(($order_details['payment_status'])) : 'pending'; ?></div> -->
                                            </div>
                                            <div>
                                                <?php
                                        ...
                                        <span class="flex items-center" style="width: 100%; display: flex;margin: 16px 0 16px 0;">
                                            ...
                                            <span class="ltr:ml-1 rtl:mr-1"> &nbsp;<?= str_replace('_', ' ', ucfirst(labels($order_details['status']))); ?> </span>
                                        </span>
                                                ...
                                                <div class=" mb-4 card-border">
                                                    <div class="" style="background-color:#f2f1f6;border-radius:0.3rem">
                                                        ...
                                                        ?> <ul class="text-dark" style="padding: 0;" class="mb-0">
                                                            <li class="flex items-center justify-between ml-2 mr-2 mt-1 pl-1 pt-3"><span><?= labels('total', 'Total') ?></span><span class="font-semibold"><span><?= !empty($order_details['total']) ? $currency . ($subtotal - $tax_amount)  : labels('no_data_found', 'No data found') ?></span></span></li>
                                                            ...
                                                            <li class="fbg-dark  flex items-center justify-between mb-lg-n5  p-3 " style="background-color: #DFDDEB;"><span> <?= labels('payable_total', "Payable Total") ?></span><span class="font-semibold"><span><?= !empty($final_total) ?  $currency . $final_total : "" ?></span></span></li>
                                                        </ul>
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <div class="col-lg-4  border-left">
                                    <div class="card text-dark border-bottom" style="border: none!important;">
                                        <form id="myForm" method="post" enctype="multipart/form-data">
                                            ...
                                            <div class="row m-0">
                                                <div class="col-md">
                                                    <div class="form-group rescheduled_date">
                                                        ...
                                                        <!-- Keep min/max in sync with advance booking rules so providers cannot pick invalid dates. -->
                                                        <input id="rescheduled_date" class="form-control" type="date" name="rescheduled_date" min="<?= esc($reschedule_min_date ?? '') ?>" <?= !empty($reschedule_max_date) ? 'max="' . esc($reschedule_max_date) . '"' : '' ?> data-min-date="<?= esc($reschedule_min_date ?? '') ?>" data-max-date="<?= esc($reschedule_max_date ?? '') ?>" data-advance-days="<?= esc(isset($advance_booking_days) ? $advance_booking_days : '') ?>" data-min-error="<?= labels(PLEASE_SELECT_UPCOMING_DATE, 'Please select upcoming date') ?>" data-max-error="<?= labels(YOU_CAN_NOT_CHOOSE_DATE_BEYOND_AVAILABLE_BOOKING_DAYS, 'You cannot choose a date beyond available booking days') ?>" data-no-advance-error="<?= labels(ADVANCED_BOOKING_FOR_THIS_PARTNER_IS_NOT_AVAILABLE, 'Advanced booking for this partner is not available') ?>">
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <p class="order-detail-p reschedulable">
                                                <?php if (isset($advance_booking_days)) : ?>
                                                    <?php if ($advance_booking_days === 0) : ?>
                                                        <?= labels(ADVANCED_BOOKING_FOR_THIS_PARTNER_IS_NOT_AVAILABLE, 'Advanced booking for this partner is not available') ?>
                                                    <?php elseif ($advance_booking_days > 0) : ?>
                                                        <?= labels('advance_booking_days', 'Advance Booking Days') ?>:
                                                        <?= esc($advance_booking_days) ?>
                                                        <?= labels(DAYS, 'days') ?>
                                                    <?php else : ?>
                                                        <?= labels('advance_booking_details', 'Advance Booking Details') ?>
                                                    <?php endif; ?>
                                                <?php endif; ?>
                                            </p>
                                        </form>
                                        ...
                                        <div class="border-bottom">
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    <ul class="nav flex-column">
                                                        <li class="nav-item">
                                                            <label for="" class="bold"> <?= !empty($order_details['date_of_service']) ? $order_details['date_of_service'] : labels('no_data_found', 'No data found') ?> </label>
                                                        </li>
                                                        <li class="nav-item">
                                                ...
                                                if (!empty($sub_order)) { ?>
                                                    <div class="row ml-0">
                                                        <strong class="ml-3">
                                                            <h6><?= labels('order_scheduled_for_the_multiple_days', "Order scheduled for the multiple days") ?></h6>
                                                        </strong>
                                                        <div class="col-md-6 ">
                                                            <ul class="nav flex-column">
                                                                <li class="nav-item">
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <ul class="nav flex-column">
                                                        ...
                                                        <li class="nav-item">
                                                            <label for="" class="bold"><?= labels('company_name', "Company Name") ?>:</label>
                                                        </li>
                                                        <li class="nav-item">
                                                            <label for="" class="bold"><?= labels('email', "Email") ?></label>
                                                        </li>
                                                ...
                                                <div class="col-md-6">
                                                    <ul class="nav flex-column">
                                                        <li class="nav-item">
                                                            <label for="" class="bold"> <?= !empty($order_details['translated_username']) ? $order_details['translated_username'] : (!empty($order_details['partner']) ? $order_details['partner'] : labels('no_data_found', 'No data found')) ?> </label>
                                                        </li>
                                                        <li class="nav-item">
                                                            <label for="" class="bold"> <?= !empty($order_details['company_name']) ? $order_details['translated_company_name'] : labels('no_data_found', 'No data found') ?> </label>
                                                        </li>
                                                        <li class="nav-item">
                                                        ...
                                                        <li class="nav-item">
                                                            <label for="" class="bold"><?= !empty($order_details['partner_address']) ? $order_details['partner_address'] : labels('no_data_found', 'No data found') ?></label>
                                                        </li>
                                                    </ul>
                                                </div>
                                            </div>
