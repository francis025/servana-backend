...
class Payments extends BaseController
{
    ...
    {
        log_message('debug', 'Stripe Payments controller (format_stripe_amount) - start');
        log_message('debug', 'Stripe Payments controller (format_stripe_amount) - amount: ' . $amount);
        log_message('debug', 'Stripe Payments controller (format_stripe_amount) - currency: ' . $currency);
        // List of zero-decimal currencies per Stripe docs
        $zeroDecimalCurrencies = ['BIF', 'CLP', 'DJF', 'GNF', 'JPY', 'KMF', 'KRW', 'MGA', 'PYG', 'RWF', 'UGX', 'VND', 'VUV', 'XAF', 'XOF', 'XPF'];

        $currency = strtoupper($currency);
        return in_array($currency, $zeroDecimalCurrencies) ? intval($amount) : intval($amount * 100);
    }
}
