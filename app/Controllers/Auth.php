...
class Auth extends BaseController
{
        ...
        if ($this->ionAuth->loggedIn()) {
            ...
            if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
                ...
                if (!empty($data)) {
                    ...
                    // This ensures admin and partner users see their preferred language directly on dashboard
                    // The preferred language is stored in preferred_language field (as set by get_language_json_data API)
                    // Set language for both admin (group_id == 1) and partner (group_id == 3) users
                    if ($data->group_id == 1 || $data->group_id == 3) {
                        try {
                            // Fetch complete user details from database to ensure we get all fields including preferred_language
                            // This is more reliable than using IonAuth's user() method which may not include all fields
                            $userDetails = fetch_details('users', ['id' => $data->id]);
                            ...
                            if (!empty($userDetails) && isset($userDetails[0]['preferred_language']) && !empty($userDetails[0]['preferred_language'])) {
                                ...
                                // This is set via get_language_json_data API which updates preferred_language field
                            ...
                            $userType = ($data->group_id == 1) ? 'admin' : 'partner';
                            log_message('error', 'Failed to set ' . $userType . ' language preference: ' . $e->getMessage());
                        }
                    }
    ...
    public function create_partner()
    {
        ...
        $partner_data = [
            ...
            'company_name' => $company,
            'is_approved' => 0,
            'slug' => generate_unique_slug($company, 'partner_details', $id),
        ];
    ...
    {
        $validation =  \Config\Services::validation();
        // Add validation rules including password confirmation match
        $validation->setRules(
            [
        ...
        if ((($user_data[0]['country_code'] == null) || ($user_data[0]['country_code'] == $this->request->getPost('store_country_code'))) && (($user_data[0]['phone'] == $identity))) {
            ...
            if ($change) {
                ...
                // Get error message from ionAuth
